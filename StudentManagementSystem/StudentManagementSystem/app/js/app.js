...
App.config(['$stateProvider', '$urlRouterProvider', '$controllerProvider', '$compileProvider', '$filterProvider', '$provide', '$ocLazyLoadProvider', 'APP_REQUIRES',
    function ($stateProvider, $urlRouterProvider, $controllerProvider, $compileProvider, $filterProvider, $provide, $ocLazyLoadProvider, appRequires) {
        ...
        $urlRouterProvider.otherwise('/page/loginPage');
        ...
        $stateProvider
            ...
            .state('app.user-save', {
                ...
                controller: 'NullController',
                resolve: resolveFor('parsley', 'inputmask', 'bwizard')
            })
            .state('app.student-detail', {
                url: '/student-detail',
                title: 'Form Standard',
                templateUrl: basepath('student-detail.html'),
                controller: 'NullController',
                resolve: resolveFor('parsley', 'inputmask', 'bwizard')
            })
            .state('app.teacher', {
                url: '/teacher',
                title: 'Form Standard',
                templateUrl: basepath('teacher.html'),
                controller: 'NullController',
                resolve: resolveFor('parsley', 'inputmask', 'bwizard')
            })
 ...
 * Targeted elements must have [data-toggle="collapse-next"]
 =========================================================*/
App.controller('SidebarController', ['$rootScope', '$scope', '$state', '$location', '$http', '$timeout', 'APP_MEDIAQUERY','storageService',
    function ($rootScope, $scope, $state, $location, $http, $timeout, mq, storageService) {
        ...
        $scope.loadSidebarMenu = function () {
            ...
            var userI = storageService.get();
            var userInfo = userI ? JSON.parse(userI) : [];
            
            if (!userInfo.ROLECODE) {
                $state.go('page.loginPage');
            } else {
                
                var menuJson = '';
                if (userInfo.ROLECODE == 'A') {
                    menuJson = 'server/sidebar-menu-A.json';
                }
                else if (userInfo.ROLECODE == 'T') {
                    menuJson = 'server/sidebar-menu-T.json';
                }
                else if (userInfo.ROLECODE == 'S') {
                    menuJson = 'server/sidebar-menu-S.json';
                }
...
App.controller('userCntrl', ['$scope', '$http', 'toaster', '$state', 'storageService', function ($scope, $http, toaster, $state, storageService) {

    //GetStudent();
    //function GetStudent() {
    //    $http.get("http://localhost:54377/api/User/GetUser")
    //        .then(function (response) {
    //            
    ...
    function RoleControl() {
        ...
        if (!userInfo.ROLECODE) {
            ...
            if (userInfo.ROLECODE == 'S' || userInfo.ROLECODE == 'T') {
                $state.go('app.dashboard');
            }
        }
    }
    ...
    function GetRoles() {
        ...
        $scope.Roles = [];
        $http.get("http://localhost:54377/api/Role/GetRoles")
            .then(function (response) {
                $scope.Roles = response.data;
            });
    }
    $scope.save_user = function (userRecord) {
        
        if (!userRecord) {
            GetPop('2');
            return;
        }
        if (!userRecord.NAME || !userRecord.SURNAME || !userRecord.PHONE || !userRecord.ROLEREF
            || !userRecord.ADDRESS || !userRecord.EMAIL || !userRecord.PASSWORD) {
            GetPop('2');
            return;
        }
        else {
            $http.post("http://localhost:54377/api/User/InsertUser", userRecord)
                .then(function (response) {
                    $scope.user = {};
                    GetPop('1');
                }, function (response) {
    ...
    $scope.save_course = function (courseRecord) {
        
        if (!courseRecord) {
            GetPop('2');
            return;
        }
        if (!courseRecord.NAME || !courseRecord.FEES || !courseRecord.DURATION) {
            GetPop('2');
            return;
        }
        else {
            $http.post("http://localhost:54377/api/Course/InsertCourse", courseRecord)
                .then(function (response) {
                    $scope.course = {};
                    GetPop('1');
                }, function (response) {
    ...
    function GetPop(statu) {
        if (statu == '1') {
            $scope.toaster = {
                type: 'success',
                title: 'Kayıt Başarıyla Kaydedildi.',
                text: ''
            };
        }
        if (statu == '2') {
...
App.controller('studentCntrl', ['$scope', '$http', 'toaster', 'userInfo', 'storageService', function ($scope, $http, toaster, userInfo, storageService) {

    GetCourse();
    function GetCourse() {
        
        var userI = storageService.get();
        var userIww = userI ? JSON.parse(userI) : [];
        $http.get("http://localhost:54377/api/Course/GetCourse")
            .then(function (response) {
    ...
    $scope.save = function (usercourseRecord) {
        
        if (!usercourseRecord) {
            GetPop('2');
            return;
        }
        if (!usercourseRecord.COURSEID) {
            GetPop('2');
            return;
        }
        else {
            ...
            if (!userInfo.USERID) {
                ...
                usercourseRecord.USERID = userInfo.USERID;
                $http.post("http://localhost:54377/api/Course/InsertUserCourse", usercourseRecord)
                    .then(function (response) {
                        $scope.course = {};
                        GetPop('1');
                    }, function (response) {
    ...
    $scope.search_content = function (COURSEID) {
        
        if (!COURSEID) {
            return;
        }
        else {
            $http.get("http://localhost:54377/api/Course/GetCourseContent", { params: { COURSEID: COURSEID } })
                .then(function (response) {
                    
                    $scope.GridContentList = response.data;
                });
        }
    };
...
App.controller('teacherCntrl', ['$scope', '$http', 'toaster', '$state', 'userInfo', 'storageService', function ($scope, $http, toaster, $state, userInfo, storageService) {
    ...
    function GetCourse() {
        ...
        var userI = userInfo.getUserName();
        $http.get("http://localhost:54377/api/Course/GetCourse")
            .then(function (response) {
    ...
    $scope.search = function (COURSEID) {
        
        if (!COURSEID) {
            return;
        }
        else {
            $http.get("http://localhost:54377/api/Course/GetCourseUserDetail", { params: { COURSEID: COURSEID } })
                .then(function (response) {
    ...
    $scope.save_detail = function (courseDetail) {
        
        if (!courseDetail) {
            GetPop('2');
            return;
        }
        if (!courseDetail.COURSEID || !courseDetail.CONTENTDETAIL || !courseDetail.LINK) {
            GetPop('2');
            return;
        }
        else {
            ...
            if (!userInfo.USERID) {
                ...
                courseDetail.USERID = userInfo.USERID;
                $http.post("http://localhost:54377/api/Course/InsertCourseDetail", courseDetail)
                    .then(function (response) {
                        $scope.courseDetail = {};
                        GetPop('1');
                    }, function (response) {
...
App.controller('loginCntrl', ['$scope', '$http', '$state', 'userInfo', 'storageService', function ($scope, $http, $state, userInfo, storageService) {
    ...
    $scope.login = function (account) {
        ...
        $http.get("http://localhost:54377/api/User/GetUserByEmail", { params: { email: $scope.account.email, password: $scope.account.password } })
            .then(function (response) {
                
                if (response.data.length == 0) {
                    $scope.authMsg = 'Email veya şifrenizi yanlış girdiniz, lütfen tekrar deneyiniz..';
                } else {
...
App.factory('storageService', ['$rootScope', function ($rootScope) {
    ...
    return {
        ...
        save: function (data) {
            sessionStorage.setItem('list', JSON.stringify(data[0]));
        }
    };
